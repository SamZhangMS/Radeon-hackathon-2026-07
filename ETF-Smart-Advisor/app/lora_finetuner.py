...
class ETFAdvisorLoRATuner:
    ...
    def __init__(
        ...
        self.base_model_name = base_model_name or QWEN_MODEL_PATH
    ...
    def _prepare_financial_dataset(self, df: pd.DataFrame) -> Dataset:
        ...
        for symbol, group in df.groupby('symbol'):
            ...
            for i in range(60, len(group) - 30, 10):
                if i + 30 >= len(group):
                    break
                
                window = group.iloc[i-60:i]
                future = group.iloc[i:i+30]
...
风险提示: 投资有风险，决策需谨慎。"""
    ...
    def _process_func(self, example: Dict[str, str]) -> Dict[str, List[int]]:
        """处理样本 - 使用 Qwen chat template"""
        ...
        messages = [
            {"role": "system", "content": "你是一个专业的 ETF 投资分析师。"},
    ...
    def train_lora(self, train_data: Union[pd.DataFrame, str], output_dir: Optional[str] = None):
        """执行 LoRA 微调"""
...
def finetune_etf_advisor(data_dir: str, output_dir: str = "./data/models/lora_etf_advisor", **kwargs):
    """快速微调"""
    ...
    return tuner.train_lora(data_dir, output_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="./data/models/lora_etf_advisor")
